...
const buildLogoPickerControl = (
	...
	const control = queryRequired(
		fragment,
		'.preview-logo-picker',
		isHtmlElement,
		'logo picker control'
	);
	const button = queryRequired(
		control,
		'#asset-logo-picker-button',
		isHtmlElement,
		'logo picker button'
	);
...
const buildZoomControls = (localize: LeLøginScreenLocaliser, zoom: number): HTMLElement => {
	...
	const controls = queryRequired(
		fragment,
		'.preview-zoom-controls',
		isHtmlElement,
		'zoom controls'
	);
	const zoomOutButton = queryRequired(
		controls,
		'#asset-zoom-out',
		isHtmlElement,
		'zoom out button'
...
const buildGreetingField = (
	...
	const control = queryRequired(
		fragment,
		'.preview-greeting-field',
		isHtmlElement,
		'greeting field'
	);
	const input = queryRequired(
		control,
		'#asset-greeting-inline',
		isTextInputElement,
		'greeting input'
	);
...
export function measureLogoPosition(
	authPreview: HTMLElement
...
export function measureGreetingPosition(
	authPreview: HTMLElement
): { leftPct: number; topPct: number } | null {
	...
	return {
		leftPct:
			((greetingRect.left + greetingRect.width / 2 - stageRect.left) / stageRect.width) * 100,
